...
function createCJSModuleWrap(url, source, isMain, loadCJS = loadCJSModule) {
  ...
  if (!exportNames.has('default'))
    wrapperNames.push('default');
  if (!exportNames.has('__cjsModule'))
    wrapperNames.push('__cjsModule');
  ...
  return new ModuleWrap(url, undefined, wrapperNames, function() {
    ...
    for (const exportName of exportNames) {
      if (!ObjectPrototypeHasOwnProperty(exports, exportName) ||
          exportName === 'default' || exportName === '__cjsModule') {
        continue;
      }
    ...
    this.setExport('__cjsModule', true);
  }, module);
}
