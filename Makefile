...
.PHONY: lint-js-fix
lint-js-fix: tools/eslint/node_modules ## Lints and fixes the JavaScript code with eslint.
...
lint-js lint-js-doc: tools/eslint/node_modules ## Lints the JavaScript code with eslint.
	@if [ "$(shell $(node_use_openssl))" != "true" ]; then \
